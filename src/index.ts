...
export interface CreateIFFOptions {
   ...
   * instead of the path generated by {@link CreateIFFOptions.generateRootDir} .
   */
...
export async function createIFF<const T extends Directory>(
  ...
  const rootDir = resolve(options.overrideRootDir ?? options.generateRootDir()); // normalize path
  ...
  const iff: CreateIFFResult<FlattenDirectory<T>> = {
    ...
    async fork(additionalDirectory, forkOptions) {
      const newRootDir = forkOptions?.overrideRootDir ?? options.generateRootDir();
